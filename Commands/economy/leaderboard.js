...
module.exports = {
    ...
    let allPlayers = db.all().map(x => {
      ...
      data.nickname = client.users.cache.get(x.ID) ? `${client.users.cache.get(data.id).username}` : `Unknown User: ${data.id}`
      return data
    })
    ...
    for (var [i, embed] of embeds.entries()) {
      embed
        ...
        .setFooter(
          `Page ${i + 1}/${embeds.length} | Sorted in descending order by coins.`
        )
    }
